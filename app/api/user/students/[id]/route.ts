...
export async function PUT(
  ...
  try {
    ...
    const uniqueFields = ['nationality_number', 'mobile', 'email'].filter(field => studentData[field]);
    ...
    // Ensure role is set
    cleanUpdateData.role = Role.STUDENT;
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2025') {
        ...
        return NextResponse.json(
          {
            error: 'Cannot update Student because they are referenced by other records',
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2025') {
        ...
        return NextResponse.json(
          {
            error: 'Cannot delete Student because they are referenced by other records',
            details: error.message
          },
          { status: 409 }
        );
